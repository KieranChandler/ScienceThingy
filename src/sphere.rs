...
impl<'a> From<&'a Sphere> for glium::index::IndicesSource<'a> {
    fn from(sphere: &'a Sphere) -> glium::index::IndicesSource<'a> {
...
impl SphereBuilder {
    ...
    {
        let vertices =
            glium::vertex::VertexBuffer::<dyn Vertex>::new(display, &self.build_vertices()?)?;
    ...
    pub fn build_vertices(&self) -> Result<Vec<dyn Vertex>, ShapeCreationError> {
        ...
        let mut vertices = Vec::<dyn Vertex>::with_capacity(total_num_verts);
